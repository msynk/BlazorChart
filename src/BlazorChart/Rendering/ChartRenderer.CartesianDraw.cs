...
{
    private void DrawGrid(ChartScene scene, Area plot, AxisScale indexScale,
        Dictionary<string, AxisScale> valueScales, List<AxisScale> leftAxes, List<AxisScale> rightAxes)
    {
    ...
    private void DrawValueAxis(ChartScene scene, Area plot, AxisScale axis, double axisPos, bool isRight, bool drawArea)
    {
        var o = axis.Options;
        var g = o.Grid;
        var tk = o.Ticks;
        ...
        foreach (var tick in axis.Ticks)
        {
            if (IsVertical)
            {
                ...
                if (tick.Minor)
                {
                    if (g.Display && drawArea && g.DrawOnChartArea)
                        scene.Background.Add(new SvgLine { X1 = plot.Left, Y1 = y, X2 = plot.Right, Y2 = y, Stroke = ColorUtil.WithAlpha(g.Color, 0.4), StrokeWidth = g.LineWidth });
                    continue;
                }
                if (g.Display && drawArea && g.DrawOnChartArea)
                    scene.Background.Add(new SvgLine
                    {
                        X1 = plot.Left, Y1 = y, X2 = plot.Right, Y2 = y,
                        Stroke = Math.Abs(tick.Value) < 1e-9 ? (g.ZeroLineColor ?? g.Color) : g.Color,
                        StrokeWidth = g.LineWidth, Dash = Svg.Dash(g.BorderDash)
                    });
                if (g.DrawTicks)
                    scene.Background.Add(new SvgLine
                    {
            ...
            else
            {
                ...
                {
                    if (g.Display && drawArea && g.DrawOnChartArea)
                        scene.Background.Add(new SvgLine { X1 = x, Y1 = plot.Top, X2 = x, Y2 = plot.Bottom, Stroke = ColorUtil.WithAlpha(g.Color, 0.4), StrokeWidth = g.LineWidth });
                    continue;
                }
                if (g.Display && drawArea && g.DrawOnChartArea)
                    scene.Background.Add(new SvgLine
        ...
        if (o.Border.Display)
        {
            var b = o.Border;
            if (IsVertical)
                scene.Background.Add(new SvgLine { X1 = axisPos, Y1 = plot.Top, X2 = axisPos, Y2 = plot.Bottom, Stroke = b.Color, StrokeWidth = b.Width, Dash = Svg.Dash(b.Dash) });
            else
                scene.Background.Add(new SvgLine { X1 = plot.Left, Y1 = plot.Bottom, X2 = plot.Right, Y2 = plot.Bottom, Stroke = b.Color, StrokeWidth = b.Width, Dash = Svg.Dash(b.Dash) });
        }
    }

    private void DrawIndexAxis(ChartScene scene, Area plot, AxisScale axis)
    {
        var o = axis.Options;
        if (!o.Display) return;
        var g = o.Grid;
        ...
        foreach (var tick in axis.Ticks)
        {
            double px = axis.Type == ScaleType.Category
            ...
            if (IsVertical)
            {
                if (g.Display && g.DrawOnChartArea)
                    scene.Background.Add(new SvgLine
                    {
                        ...
                        Stroke = g.Color, StrokeWidth = g.LineWidth, Dash = Svg.Dash(g.BorderDash)
                    });
                if (g.DrawTicks)
                    scene.Background.Add(new SvgLine
                    {
        ...
        if (o.Border.Display)
        {
            ...
            if (IsVertical)
                ...
                scene.Background.Add(new SvgLine { X1 = plot.Left, Y1 = plot.Top, X2 = plot.Left, Y2 = plot.Bottom, Stroke = b.Color, StrokeWidth = b.Width, Dash = Svg.Dash(b.Dash) });
        }
    }

    private bool HasBars() => _data.Datasets.Where((d, i) => !IsHidden(i, d)).Any(d => EffectiveType(d) == ChartType.Bar);

    /// <summary>Draws a secondary x-axis (ticks/labels/border/title) at a baseline outside the plot.</summary>
    private void DrawSecondaryXAxis(ChartScene scene, Area plot, AxisScale axis, double baselineY, bool atBottom)
    {
        var o = axis.Options;
        if (!o.Display) return;
        var g = o.Grid;
        ...
        foreach (var tick in axis.Ticks)
        {
            ...
            if (g.Display && g.DrawOnChartArea && !tick.Minor)
                scene.Background.Add(new SvgLine { X1 = px, Y1 = plot.Top, X2 = px, Y2 = plot.Bottom, Stroke = g.Color, StrokeWidth = g.LineWidth, Dash = Svg.Dash(g.BorderDash) });
            if (g.DrawTicks)
                scene.Background.Add(new SvgLine { X1 = px, Y1 = baselineY, X2 = px, Y2 = baselineY + dir * g.TickLength, Stroke = g.TickColor, StrokeWidth = g.LineWidth });
        ...
        if (o.Border.Display)
            scene.Background.Add(new SvgLine { X1 = plot.Left, Y1 = baselineY, X2 = plot.Right, Y2 = baselineY, Stroke = o.Border.Color, StrokeWidth = o.Border.Width, Dash = Svg.Dash(o.Border.Dash) });
