...
{
    private void RenderRadar(ChartScene scene)
    {
        ...
        var rScale = new AxisScale(rOpts, horizontal: false);
        ...
        if (rOpts.Display && rOpts.Grid.Display)
        {
            foreach (var t in rScale.Ticks)
            {
                ...
                {
                    var poly = new SvgPolygon { Fill = "none", Stroke = rOpts.Grid.Color, StrokeWidth = rOpts.Grid.LineWidth };
                    for (int i = 0; i < n; i++)
                    {
        ...
        for (int i = 0; i < n; i++)
        {
            ...
            if (rOpts.Display && rOpts.AngleLines)
                scene.Background.Add(new SvgLine
                {
                    ...
                    Dash = Svg.Dash(rOpts.AngleLineDash)
                });
        ...
        for (int d = 0; d < _data.Datasets.Count; d++)
        {
            ...
            string fill = ds.FillColor ?? ColorUtil.WithAlpha(border, 0.2);
            ...
            scene.Background.Add(new SvgPolygon
            {
                Points = verts.Select(p => (p.x, p.y)).ToList(),
                Fill = ds.Fill != FillMode.None ? fill : "none",
                Stroke = border,
