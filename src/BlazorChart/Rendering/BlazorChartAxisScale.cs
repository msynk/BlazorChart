...
/// </summary>
public sealed class BlazorChartAxisScale
{
    public BlazorChartScaleOptions Options { get; }
    public BlazorChartScaleType Type => Options.Type;
    public double Min { get; private set; }
    public double Max { get; private set; }
    public bool Horizontal { get; }
    public List<BlazorChartAxisTick> Ticks { get; } = new();
    ...
    public BlazorChartAxisScale(BlazorChartScaleOptions options, bool horizontal, List<string>? categories = null)
    {
        Options = options;
    ...
    {
        if (Type == BlazorChartScaleType.Category)
        {
        ...
        if (Options.Grace > 0 && Type != BlazorChartScaleType.Logarithmic)
        {
        ...
        if (Type == BlazorChartScaleType.Logarithmic)
        {
    ...
    {
        double count = Type == BlazorChartScaleType.Category
    ...
    private void BuildTicks()
    {
        ...
        switch (Type)
        {
            case BlazorChartScaleType.Category:
                BuildCategoryTicks();
                break;
            case BlazorChartScaleType.Logarithmic:
                BuildLogTicks();
                break;
            case BlazorChartScaleType.Time:
                BuildTimeTicks();
                break;
            default:
                BuildLinearTicks();
    ...
    private void BuildTimeTicks()
    {
        ...
        foreach (var (value, label) in BlazorChartTimeAxis.Ticks(Min, Max, Options.TimeUnit, Options.TimeFormat, maxTicks))
            Ticks.Add(new BlazorChartAxisTick(value, label, PixelFor(value)));
    }
    ...
    private void BuildCategoryTicks()
    {
        ...
        for (int i = start; i <= end; i++)
        {
            ...
            Ticks.Add(new BlazorChartAxisTick(i, _categories[i], px));
        }
    }
    ...
    private void BuildLinearTicks()
    {
        ...
        for (double v = niceMin; v <= niceMax + rawStep * 0.5; v += rawStep)
        {
            ...
            Ticks.Add(new BlazorChartAxisTick(val, FormatValue(val, decimals), PixelFor(val)));
        }
    }
    ...
    private void BuildLogTicks()
    {
        ...
        for (double e = lo; e <= hi; e++)
        {
            ...
            if (major >= Min - 1e-9 && major <= Max + 1e-9)
                Ticks.Add(new BlazorChartAxisTick(major, FormatValue(major, 0), PixelFor(major)));
            ...
            if (Options.Ticks.Display)
                for (int m = 2; m <= 9; m++)
                {
                    ...
                    Ticks.Add(new BlazorChartAxisTick(minor, "", PixelFor(minor), Minor: true));
                }
        }
        if (Ticks.Count == 0)
        {
            Ticks.Add(new BlazorChartAxisTick(Min, FormatValue(Min, 0), PixelFor(Min)));
            Ticks.Add(new BlazorChartAxisTick(Max, FormatValue(Max, 0), PixelFor(Max)));
        }
    }
