...
public readonly record struct AxisTick(double Value, string Label, double Pixel, bool Minor = false);
...
{
    public ScaleOptions Options { get; }
    public ScaleType Type => Options.Type;
    ...
    public AxisScale(ScaleOptions options, bool horizontal, List<string>? categories = null)
    {
        Options = options;
    ...
    public void SetDataRange(double dataMin, double dataMax)
    {
        ...
        if (Options.Grace > 0 && Type != ScaleType.Logarithmic)
        {
    ...
    {
        double count = Type == ScaleType.Category
    ...
    private void BuildTimeTicks()
    {
        ...
        foreach (var (value, label) in TimeAxis.Ticks(Min, Max, Options.TimeUnit, Options.TimeFormat, maxTicks))
            Ticks.Add(new AxisTick(value, label, PixelFor(value)));
    }
    ...
    private void BuildCategoryTicks()
    {
        ...
        for (int i = start; i <= end; i++)
        {
            ...
            Ticks.Add(new AxisTick(i, _categories[i], px));
        }
    }
    ...
    private void BuildLinearTicks()
    {
        ...
        for (double v = niceMin; v <= niceMax + rawStep * 0.5; v += rawStep)
        {
            ...
            Ticks.Add(new AxisTick(val, FormatValue(val, decimals), PixelFor(val)));
        }
    }
    ...
    private void BuildLogTicks()
    {
        ...
        for (double e = lo; e <= hi; e++)
        {
            ...
            if (major >= Min - 1e-9 && major <= Max + 1e-9)
                Ticks.Add(new AxisTick(major, FormatValue(major, 0), PixelFor(major)));
            ...
            if (Options.Ticks.Display)
                for (int m = 2; m <= 9; m++)
                {
                    ...
                    Ticks.Add(new AxisTick(minor, "", PixelFor(minor), Minor: true));
                }
        }
        if (Ticks.Count == 0)
        {
            Ticks.Add(new AxisTick(Min, FormatValue(Min, 0), PixelFor(Min)));
            Ticks.Add(new AxisTick(Max, FormatValue(Max, 0), PixelFor(Max)));
        }
    }
