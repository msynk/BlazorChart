...
{
    private bool IsVertical => _options.IndexAxis == BlazorChartIndexAxis.X;

    private void RenderCartesian(BlazorChartScene scene)
    {
        ...
        bool indexIsCategory = indexScaleOpts.Type == BlazorChartScaleType.Category;

        // Value (y) axes used by datasets.
        var leftAxes = new List<BlazorChartAxisScale>();
        var rightAxes = new List<BlazorChartAxisScale>();
        var valueScales = new Dictionary<string, BlazorChartAxisScale>();

        foreach (var (id, so) in _options.Scales)
        {
            if (xIds.Contains(id) || so.Type == BlazorChartScaleType.RadialLinear) continue;
            var (mn, mx) = ComputeValueExtent(id);
            var scale = new BlazorChartAxisScale(so, horizontal: !IsVertical);
            if (so.Type != BlazorChartScaleType.Category && _state.AxisRanges.TryGetValue(id, out var ov))
                scale.Forced = ov;
            scale.SetDataRange(mn, mx);
            scale.SetPixelRange(0, 100); // provisional, for tick labels
            valueScales[id] = scale;
            if (so.Type != BlazorChartScaleType.Category) scene.ZoomableAxes.Add(id);
            if (!so.Display) continue;
            if ((so.Position ?? BlazorChartPosition.Left) == BlazorChartPosition.Right) rightAxes.Add(scale);
            else leftAxes.Add(scale);
        }

        // X axes ("x" is the primary index scale; others are secondary, point-bound).
        var xScales = new Dictionary<string, BlazorChartAxisScale>();
        var bottomXAxes = new List<BlazorChartAxisScale>();
        var topXAxes = new List<BlazorChartAxisScale>();
        BlazorChartAxisScale indexScale = default!;

        foreach (var id in xIds.OrderBy(s => s == "x" ? 0 : 1))
        {
            var so = _options.Scales[id];
            BlazorChartAxisScale xs;
            if (id == "x" && indexIsCategory)
            {
                xs = new BlazorChartAxisScale(so, horizontal: IsVertical, categories: _data.Labels);
            ...
            else
            {
                ...
                xs = new BlazorChartAxisScale(so, horizontal: IsVertical);
            ...
            if (so.Display)
            {
                if ((so.Position ?? BlazorChartPosition.Bottom) == BlazorChartPosition.Top) topXAxes.Add(xs);
        ...
        var plot = new BlazorChartArea(area.Left + leftReserve, area.Top + topReserve, area.Right - rightReserve, area.Bottom - bottomReserve);
        ...
        BlazorChartAxisScale XScaleFor(BlazorChartDataset ds)
        ...
        var barItems = ordered.Where(t => EffectiveType(t.d) == BlazorChartType.Bar && !IsHidden(t.i, t.d)).ToList();
        ...
        var stackedLines = ordered
            .Where(t => EffectiveType(t.d) == BlazorChartType.Line && !IsHidden(t.i, t.d)
    ...
    private BlazorChartType EffectiveType(BlazorChartDataset ds) => ds.Type ?? _config.Type;
    private bool IsHidden(int i, BlazorChartDataset ds) => ds.Hidden || _state.IsDatasetHidden(i);

    private double ReserveIndexAxis(BlazorChartAxisScale scale)
    {
        var o = scale.Options;
        ...
        if (o.Ticks.Display)
        {
            ...
            else
            {
                ...
                foreach (var t in scale.Ticks)
                    maxLabel = Math.Max(maxLabel, BlazorChartTextMeasure.Width(t.Label, o.Ticks.Font.Size));
    ...
    private static double ComputeIndexLabelRotation(BlazorChartAxisScale scale, double availWidth)
    {
        ...
        double maxLabel = 0;
        foreach (var t in scale.Ticks)
            maxLabel = Math.Max(maxLabel, BlazorChartTextMeasure.Width(t.Label, tk.Font.Size, tk.Font.Weight));
    ...
    private double ReserveValueAxis(BlazorChartAxisScale scale)
    {
        var o = scale.Options;
        double maxLabel = 0;
        if (o.Ticks.Display)
