...
/// </summary>
public sealed class CenterTextPlugin : IChartPlugin
{
    ...
    public ChartFont Font { get; set; } = new() { Size = 28, Weight = "bold" };
    public ChartFont SubtextFont { get; set; } = new() { Size = 13 };
    ...
    public CenterTextPlugin(string text, string? subtext = null)
    {
        Text = text;
        Subtext = subtext;
    }
    ...
    public void AfterDatasetsDraw(PluginContext ctx)
    {
        ...
        if (FitToCutout && ctx.InnerRadius > 0)
        {
            ...
            double w = TextMeasure.Width(Text, size, Font.Weight);
