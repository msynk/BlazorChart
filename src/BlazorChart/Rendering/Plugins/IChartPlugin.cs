...
/// <see cref="PluginContext"/> with the computed scene, plotting area and scale conversions,
...
public interface IChartPlugin
{
    ...
    void BeforeDatasetsDraw(PluginContext ctx) { }

    /// <summary>Called after datasets are drawn.</summary>
    void AfterDatasetsDraw(PluginContext ctx) { }
}

/// <summary>Context passed to plugins, exposing the scene, area and scale conversions.</summary>
public sealed class PluginContext
{
    public required ChartScene Scene { get; init; }
    public required ChartConfig Config { get; init; }
    public Area? Plot { get; init; }
    ...
    internal AxisScale? IndexScale { get; init; }
    internal Dictionary<string, AxisScale>? ValueScales { get; init; }
    ...
    public void AddBehind(SvgNode node) => Scene.Background.Add(node);
    public void AddFront(SvgNode node) => Scene.Foreground.Add(node);
}
