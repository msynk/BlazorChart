...
{
    private void DrawBars(ChartScene scene, Area plot, AxisScale indexScale,
        Dictionary<string, AxisScale> valueScales, List<(ChartDataset d, int i)> barItems, bool indexIsCategory)
    {
        ...
        foreach (var (ds, i) in barItems)
        {
            ...
            for (int di = 0; di < count; di++)
            {
                ...
                SvgNode? borderNode = null;
                if (!perCorner && ds.BorderWidth > 0)
                {
                    var skip = ResolveSkip(isRange ? BorderSkipped.None : ds.BorderSkipped, IsVertical, signFinal);
                    if (skip == BorderSkipped.None)
                    {
                        rect.Stroke = border;
                ...
                scene.Elements.Add(new DataElement
                {
                    ...
                    Tooltip = new TooltipInfo
                    {
                        ...
                        Items = { new TooltipItem { Color = bg, Text = text } }
                    }
                });
    ...
    private static BorderSkipped ResolveSkip(BorderSkipped s, bool vertical, int sign) => s switch
    {
        BorderSkipped.Start => vertical ? (sign >= 0 ? BorderSkipped.Bottom : BorderSkipped.Top)
                                        : (sign >= 0 ? BorderSkipped.Left : BorderSkipped.Right),
        BorderSkipped.End => vertical ? (sign >= 0 ? BorderSkipped.Top : BorderSkipped.Bottom)
                                      : (sign >= 0 ? BorderSkipped.Right : BorderSkipped.Left),
        _ => s
    };

    /// <summary>Builds an open border path for a bar, omitting the skipped edge.</summary>
    private static string BarBorderPath(SvgRect r, BorderSkipped skip)
    {
        double x1 = r.X, y1 = r.Y, x2 = r.X + r.Width, y2 = r.Y + r.Height;
        var sb = new StringBuilder();
        void Edge(double ax, double ay, double bx, double by)
            => sb.Append($"M {Svg.N(ax)} {Svg.N(ay)} L {Svg.N(bx)} {Svg.N(by)} ");
        if (skip != BorderSkipped.Top) Edge(x1, y1, x2, y1);
        if (skip != BorderSkipped.Right) Edge(x2, y1, x2, y2);
        if (skip != BorderSkipped.Bottom) Edge(x2, y2, x1, y2);
        if (skip != BorderSkipped.Left) Edge(x1, y2, x1, y1);
        return sb.ToString().Trim();
    }

    private void DrawLine(ChartScene scene, Area plot, AxisScale indexScale, AxisScale vScale,
        ChartDataset ds, int dsIndex, bool indexIsCategory, bool centered)
    {
    ...
    private void FlushLine(ChartScene scene, Area plot, AxisScale vScale, ChartDataset ds, int dsIndex,
        List<(double x, double y, int di, double v)> pts,
        AxisScale? indexScale = null, bool indexIsCategory = false, bool centered = false)
    {
        if (pts.Count == 0) return;

        var dec = _options.Plugins.Decimation;
        if (dec.Enabled && pts.Count > dec.Threshold && dec.Samples >= 2 && dec.Samples < pts.Count)
            pts = Decimation.Lttb(pts, dec.Samples);
        ...
        if (ds.Fill != FillMode.None && ds.ShowLine)
        {
            string? fillD = null;

            // Fill to another dataset's line (range area).
            if (ds.Fill == FillMode.Dataset && ds.FillTargetIndex is { } ti
            ...
            if (fillD is null)
            {
                ...
                {
                    FillMode.Start => Math.Min(vScale.Min, vScale.Max),
                    FillMode.End => Math.Max(vScale.Min, vScale.Max),
                    FillMode.Value => ds.FillValue ?? 0,
                    _ => Math.Clamp(0, Math.Min(vScale.Min, vScale.Max), Math.Max(vScale.Min, vScale.Max))
                };
                double baseY = vScale.PixelFor(baseVal);
                fillD = d + $" L {Svg.N(xy[^1].x)} {Svg.N(baseY)} L {Svg.N(xy[0].x)} {Svg.N(baseY)} Z";
            }

            scene.Series.Add(new SvgPath { D = fillD, Fill = ResolveFill(scene, ds, border, plot), Stroke = null });
        }
        ...
        if (ds.ShowLine)
        {
            if (ds.Segment is { } seg)
            {
                ...
                for (int k = 0; k < pts.Count - 1; k++)
                {
                    var a = pts[k];
                    var b = pts[k + 1];
                    var sctx = new SegmentContext(a.di, b.di, a.v, b.v);
                    ...
                    scene.Series.Add(new SvgPath
                    {
                        D = $"M {Svg.N(a.x)} {Svg.N(a.y)} L {Svg.N(b.x)} {Svg.N(b.y)}",
                        Fill = "none", Stroke = color, StrokeWidth = width,
                        Dash = dash is null ? "" : Svg.Dash(dash),
            ...
            else
            {
                scene.Series.Add(new SvgPath
                {
                    ...
                    Dash = Svg.Dash(ds.BorderDash), LineCap = ds.BorderCapStyle, LineJoin = ds.BorderJoinStyle
                });
            }
        }

        if (ds.PointRadius > 0 || ds.PointStyle != PointStyle.None)
    ...
    private string ResolveFill(ChartScene scene, ChartDataset ds, string border, Area plot)
    {
        ...
        return ds.FillColor ?? ColorUtil.WithAlpha(border, 0.2);
    }
    ...
    private List<(double x, double y, int di, double v)> ComputeLinePoints(
        ChartDataset ds, AxisScale indexScale, AxisScale vScale, bool indexIsCategory, bool centered)
    {
        var pts = new List<(double x, double y, int di, double v)>();
        if (ds.Points is { } points)
        {
    ...
    private static string AreaBetween(List<(double x, double y)> top, double tension, SteppedLine stepped,
        List<(double x, double y)> bottom)
    {
        ...
        rev.Reverse();
        sb.Append(' ').Append('L').Append(' ').Append(Svg.N(rev[0].x)).Append(' ').Append(Svg.N(rev[0].y));
    ...
    private void DrawStackedAreas(ChartScene scene, Area plot, AxisScale indexScale,
        Dictionary<string, AxisScale> valueScales, List<(ChartDataset d, int i)> items,
    ...
    private void DrawScatter(ChartScene scene, Area plot, AxisScale indexScale, AxisScale vScale,
        ChartDataset ds, int dsIndex, bool bubble)
    {
    ...
    private void AddPoint(ChartScene scene, ChartDataset ds, int dsIndex, int di,
        double x, double y, double value, double radius, string border, double? xValue = null)
    {
        ...
        var shape = PointShapes.Build(style, x, y, r, fill, stroke, bw);
        if (shape is null) return;

        bool cartesian = _config.Type is not (ChartType.Pie or ChartType.Doughnut or ChartType.PolarArea or ChartType.Radar);
        ...
        scene.Elements.Add(new DataElement
        {
            ...
            Tooltip = new TooltipInfo
            {
                ...
                AnchorX = x,
                AnchorY = y,
                Items = { new TooltipItem { Color = fill, Text = text, PointStyle = style } }
            }
        });
    }

    private void AddDataLabel(ChartScene scene, double value, double x, double y, int dsIndex = 0, int dataIndex = 0)
    {
        ...
        {
            double w = TextMeasure.Width(text, dl.Font.Size, dl.Font.Weight) + dl.Padding * 2;
            double h = dl.Font.Size + dl.Padding * 2;
            scene.Foreground.Add(new SvgRect
            {
                X = x - w / 2, Y = y - h / 2, Width = w, Height = h, Rx = dl.BorderRadius, Fill = bgc
            });
        }

        scene.Foreground.Add(new SvgText
        {
            X = x, Y = y,
            Text = text,
    ...
    private static string BuildPath(List<(double x, double y)> p, double tension, SteppedLine stepped,
        CubicInterpolationMode mode = CubicInterpolationMode.Default)
    {
        if (p.Count == 0) return "";
        var sb = new StringBuilder();
        sb.Append($"M {Svg.N(p[0].x)} {Svg.N(p[0].y)}");
        if (p.Count == 1) return sb.ToString();

        if (stepped != SteppedLine.False)
        {
            for (int i = 1; i < p.Count; i++)
            {
                ...
                switch (stepped)
                {
                    case SteppedLine.Before:
                        sb.Append($" L {Svg.N(a.x)} {Svg.N(b.y)} L {Svg.N(b.x)} {Svg.N(b.y)}");
                        break;
                    case SteppedLine.After:
                        sb.Append($" L {Svg.N(b.x)} {Svg.N(a.y)} L {Svg.N(b.x)} {Svg.N(b.y)}");
                        break;
                    default: // middle
                        double mx = (a.x + b.x) / 2;
                        sb.Append($" L {Svg.N(mx)} {Svg.N(a.y)} L {Svg.N(mx)} {Svg.N(b.y)} L {Svg.N(b.x)} {Svg.N(b.y)}");
                        break;
                }
            }
            return sb.ToString();
        ...
        if (mode == CubicInterpolationMode.Monotone && p.Count > 2)
            return MonotonePath(p);

        if (tension <= 0)
        {
            for (int i = 1; i < p.Count; i++)
                sb.Append($" L {Svg.N(p[i].x)} {Svg.N(p[i].y)}");
            return sb.ToString();
        }
        ...
        for (int i = 0; i < p.Count - 1; i++)
        {
            ...
            sb.Append($" C {Svg.N(c1x)} {Svg.N(c1y)}, {Svg.N(c2x)} {Svg.N(c2y)}, {Svg.N(p2.x)} {Svg.N(p2.y)}");
        }
        return sb.ToString();
    }
    ...
    private static string MonotonePath(List<(double x, double y)> p)
    {
        ...
        for (int i = 0; i < n - 1; i++)
        {
            ...
            sb.Append($" C {Svg.N(c1x)} {Svg.N(c1y)}, {Svg.N(c2x)} {Svg.N(c2y)}, {Svg.N(p[i + 1].x)} {Svg.N(p[i + 1].y)}");
        }
        return sb.ToString();
    }

    /// <summary>Builds a rounded-rectangle path with per-corner radii.</summary>
    private static string RoundedRectPath(double x, double y, double w, double h, BorderRadiusCorners c)
    {
        ...
        return $"M {Svg.N(x + tl)} {Svg.N(y)} " +
               $"L {Svg.N(x + w - tr)} {Svg.N(y)} A {Svg.N(tr)} {Svg.N(tr)} 0 0 1 {Svg.N(x + w)} {Svg.N(y + tr)} " +
               $"L {Svg.N(x + w)} {Svg.N(y + h - br)} A {Svg.N(br)} {Svg.N(br)} 0 0 1 {Svg.N(x + w - br)} {Svg.N(y + h)} " +
               $"L {Svg.N(x + bl)} {Svg.N(y + h)} A {Svg.N(bl)} {Svg.N(bl)} 0 0 1 {Svg.N(x)} {Svg.N(y + h - bl)} " +
               $"L {Svg.N(x)} {Svg.N(y + tl)} A {Svg.N(tl)} {Svg.N(tl)} 0 0 1 {Svg.N(x + tl)} {Svg.N(y)} Z";
    }
}
