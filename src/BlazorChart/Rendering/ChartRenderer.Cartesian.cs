...
{
    private bool IsVertical => _options.IndexAxis == IndexAxis.X;

    private void RenderCartesian(ChartScene scene)
    {
        ...
        bool indexIsCategory = indexScaleOpts.Type == ScaleType.Category;

        // Value (y) axes used by datasets.
        var leftAxes = new List<AxisScale>();
        var rightAxes = new List<AxisScale>();
        var valueScales = new Dictionary<string, AxisScale>();

        foreach (var (id, so) in _options.Scales)
        {
            if (xIds.Contains(id) || so.Type == ScaleType.RadialLinear) continue;
            var (mn, mx) = ComputeValueExtent(id);
            var scale = new AxisScale(so, horizontal: !IsVertical);
            if (so.Type != ScaleType.Category && _state.AxisRanges.TryGetValue(id, out var ov))
                scale.Forced = ov;
            scale.SetDataRange(mn, mx);
            scale.SetPixelRange(0, 100); // provisional, for tick labels
            valueScales[id] = scale;
            if (so.Type != ScaleType.Category) scene.ZoomableAxes.Add(id);
            if (!so.Display) continue;
            if ((so.Position ?? Position.Left) == Position.Right) rightAxes.Add(scale);
            else leftAxes.Add(scale);
        }

        // X axes ("x" is the primary index scale; others are secondary, point-bound).
        var xScales = new Dictionary<string, AxisScale>();
        var bottomXAxes = new List<AxisScale>();
        var topXAxes = new List<AxisScale>();
        AxisScale indexScale = default!;
        ...
        foreach (var id in xIds.OrderBy(s => s == "x" ? 0 : 1))
        {
            ...
            {
                xs = new AxisScale(so, horizontal: IsVertical, categories: _data.Labels);
            ...
            else
            {
                ...
                xs = new AxisScale(so, horizontal: IsVertical);
            ...
            if (so.Display)
            {
                if ((so.Position ?? Position.Bottom) == Position.Top) topXAxes.Add(xs);
        ...
        var plot = new Area(area.Left + leftReserve, area.Top + topReserve, area.Right - rightReserve, area.Bottom - bottomReserve);
        ...
        AxisScale XScaleFor(ChartDataset ds)
        ...
        var barItems = ordered.Where(t => EffectiveType(t.d) == ChartType.Bar && !IsHidden(t.i, t.d)).ToList();
        ...
        var stackedLines = ordered
            .Where(t => EffectiveType(t.d) == ChartType.Line && !IsHidden(t.i, t.d)
        ...
        foreach (var (ds, i) in ordered)
        {
            ...
            switch (type)
            {
                case ChartType.Line when !stackedSet.Contains(i):
    ...
    private ChartType EffectiveType(ChartDataset ds) => ds.Type ?? _config.Type;
    private bool IsHidden(int i, ChartDataset ds) => ds.Hidden || _state.IsDatasetHidden(i);

    private double ReserveIndexAxis(AxisScale scale)
    {
        var o = scale.Options;
        ...
        if (o.Ticks.Display)
        {
            ...
            else
            {
                ...
                foreach (var t in scale.Ticks)
                    maxLabel = Math.Max(maxLabel, TextMeasure.Width(t.Label, o.Ticks.Font.Size));
    ...
    private static double ComputeIndexLabelRotation(AxisScale scale, double availWidth)
    {
        ...
        double maxLabel = 0;
        foreach (var t in scale.Ticks)
            maxLabel = Math.Max(maxLabel, TextMeasure.Width(t.Label, tk.Font.Size, tk.Font.Weight));
    ...
    private double ReserveValueAxis(AxisScale scale)
    {
        var o = scale.Options;
        double maxLabel = 0;
        if (o.Ticks.Display)
