...
{
    private void RenderRadar(BlazorChartScene scene)
    {
        ...
        var rScale = new BlazorChartAxisScale(rOpts, horizontal: false);
        ...
        if (rOpts.Display && rOpts.Grid.Display)
        {
            foreach (var t in rScale.Ticks)
            {
                ...
                {
                    var poly = new BlazorChartSvgPolygon { Fill = "none", Stroke = rOpts.Grid.Color, StrokeWidth = rOpts.Grid.LineWidth };
                    for (int i = 0; i < n; i++)
                    {
        ...
        for (int i = 0; i < n; i++)
        {
            ...
            if (rOpts.Display && rOpts.AngleLines)
                scene.Background.Add(new BlazorChartSvgLine
                {
                    ...
                    Dash = BlazorChartSvg.Dash(rOpts.AngleLineDash)
                });
        ...
        for (int d = 0; d < _data.Datasets.Count; d++)
        {
            ...
            string fill = ds.FillColor ?? BlazorChartColorUtil.WithAlpha(border, 0.2);
