...
{
    private void RenderCircular(BlazorChartScene scene)
    {
        ...
        if (_config.Type == BlazorChartType.PolarArea)
        {
        ...
        double cutout = _config.Type == BlazorChartType.Doughnut ? _options.CutoutPercentage / 100.0 : 0;
        ...
        for (int ri = 0; ri < datasets.Count; ri++)
        {
            ...
            for (int i = 0; i < ds.Data.Count; i++)
            {
                ...
                var path = new BlazorChartSvgPath
                {
                    D = ArcPath(cx + ox, cy + oy, inner, outer, a0, a1),
                    Fill = active ? BlazorChartColorUtil.Adjust(bg, 0.08) : bg,
                ...
                scene.Elements.Add(new BlazorChartDataElement
                {
                    ...
                    Tooltip = new BlazorChartTooltipInfo
                    {
                        ...
                        Items = { new BlazorChartTooltipItem { Color = bg, Text = $"{v.ToString("0.##", CultureInfo.InvariantCulture)} ({pct.ToString("0.#", CultureInfo.InvariantCulture)}%)" } }
                    }
                });
    ...
    private void RenderPolarArea(BlazorChartScene scene, double cx, double cy, double maxR)
    {
        ...
        var rScale = new BlazorChartAxisScale(_options.Scales["r"], horizontal: false);
        ...
        if (rOpts.Display && rOpts.Grid.Display)
        {
            foreach (var t in rScale.Ticks)
            {
                double rr = t.Pixel;
                if (rr <= 0) continue;
                scene.Background.Add(new BlazorChartSvgCircle { Cx = cx, Cy = cy, R = rr, Fill = "none", Stroke = rOpts.Grid.Color, StrokeWidth = rOpts.Grid.LineWidth });
                if (rOpts.Ticks.Display)
                {
                    if (rOpts.ShowLabelBackdrop)
                    {
                        double w = BlazorChartTextMeasure.Width(t.Label, rOpts.Ticks.Font.Size) + 4;
                        scene.Background.Add(new BlazorChartSvgRect { X = cx + 2, Y = cy - rr - rOpts.Ticks.Font.Size * 0.55, Width = w, Height = rOpts.Ticks.Font.Size + 2, Fill = rOpts.BackdropColor });
                    }
                    scene.Background.Add(new BlazorChartSvgText { X = cx + 4, Y = cy - rr, Text = t.Label, Fill = rOpts.Ticks.Color, FontSize = rOpts.Ticks.Font.Size, FontFamily = rOpts.Ticks.Font.Family, Anchor = "start", Baseline = "central" });
                }
            }
        }
        ...
        for (int i = 0; i < n; i++)
        {
            ...
            var path = new BlazorChartSvgPath
            {
                ...
                Fill = BlazorChartColorUtil.WithAlpha(active ? BlazorChartColorUtil.Adjust(bg, -0.1) : bg, 0.7),
            ...
            scene.Elements.Add(new BlazorChartDataElement
            {
                ...
                Tooltip = new BlazorChartTooltipInfo
                {
                    ...
                    Items = { new BlazorChartTooltipItem { Color = bg, Text = v.ToString("0.##", CultureInfo.InvariantCulture) } }
                }
            });
        }
    ...
    private static string ArcPath(double cx, double cy, double inner, double outer, double a0, double a1)
    {
        ...
        if (inner <= 0.01)
        {
            return $"M {BlazorChartSvg.N(cx)} {BlazorChartSvg.N(cy)} L {BlazorChartSvg.N(x0o)} {BlazorChartSvg.N(y0o)} " +
                   $"A {BlazorChartSvg.N(outer)} {BlazorChartSvg.N(outer)} 0 {large} 1 {BlazorChartSvg.N(x1o)} {BlazorChartSvg.N(y1o)} Z";
        }

        double x0i = cx + inner * Math.Cos(a0), y0i = cy + inner * Math.Sin(a0);
        double x1i = cx + inner * Math.Cos(a1), y1i = cy + inner * Math.Sin(a1);
        return $"M {BlazorChartSvg.N(x0o)} {BlazorChartSvg.N(y0o)} " +
               $"A {BlazorChartSvg.N(outer)} {BlazorChartSvg.N(outer)} 0 {large} 1 {BlazorChartSvg.N(x1o)} {BlazorChartSvg.N(y1o)} " +
               $"L {BlazorChartSvg.N(x1i)} {BlazorChartSvg.N(y1i)} " +
               $"A {BlazorChartSvg.N(inner)} {BlazorChartSvg.N(inner)} 0 {large} 0 {BlazorChartSvg.N(x0i)} {BlazorChartSvg.N(y0i)} Z";
    }
}
