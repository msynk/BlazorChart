...
/// </summary>
public partial class BlazorChart : ComponentBase, IAsyncDisposable
{
    /// <summary>Full configuration (type + data + options). Takes precedence when set.</summary>
    [Parameter] public BlazorChartConfig? Config { get; set; }

    [Parameter] public BlazorChartType Type { get; set; } = BlazorChartType.Line;
    [Parameter] public BlazorChartData? Data { get; set; }
    [Parameter] public BlazorChartOptions? Options { get; set; }
    ...
    [Parameter] public RenderFragment<BlazorChartTooltipContext>? TooltipTemplate { get; set; }
    ...
    private readonly BlazorChartRenderState _state = new();
    private BlazorChartConfig _config = new();
    private BlazorChartScene _scene = new();
    ...
    private BlazorChartDataElement? _hovered;
    private readonly HashSet<BlazorChartDataElement> _active = new();
    private BlazorChartTooltipInfo? _activeTooltip;
    private BlazorChartTooltipContext? _tooltipContext;
    private readonly List<BlazorChartSvgNode> _hoverNodes = new();
    ...
    private DotNetObjectReference<BlazorChart>? _dotRef;
    ...
    {
        _config = Config ?? new BlazorChartConfig(Type, Data ?? new BlazorChartData(), Options ?? new BlazorChartOptions());
        Recompute();
    }
    ...
    private IEnumerable<string> AxesForMode()
    {
        ...
        {
            bool isX = id == "x";
            if (mode == BlazorChartZoomMode.X && !isX) continue;
            if (mode == BlazorChartZoomMode.Y && isX) continue;
            yield return id;
        }
    }
    ...
    {
        bool circular = _config.Type is BlazorChartType.Pie or BlazorChartType.Doughnut
            or BlazorChartType.PolarArea or BlazorChartType.Radar;
        ...
        _scene = new BlazorChartRenderer(_config, _state, _vw, _vh).Render();
        ClearHover();
        _focusIndex = -1;
    ...
    private void OnEnter(BlazorChartDataElement e)
    {
        _hovered = e;
        BuildHover(e);
    }
    ...
    private void BuildHover(BlazorChartDataElement e)
    {
        _active.Clear();
        _hoverNodes.Clear();
        var tip = _config.Options.Plugins.Tooltip;

        IEnumerable<BlazorChartDataElement> group = tip.Mode switch
        {
            BlazorChartInteractionMode.Index or BlazorChartInteractionMode.X or BlazorChartInteractionMode.Y when !_scene.IsRadialOrCircular
                => _scene.Elements.Where(x => x.DataIndex == e.DataIndex),
            BlazorChartInteractionMode.Dataset
        ...
        // Combined tooltip.
        var combined = new BlazorChartTooltipInfo
        {
        ...
        {
            var items = ordered.Select(a => new BlazorChartTooltipItemContext
            {
        ...
        // Positioner.
        if (tip.Position == BlazorChartTooltipPositioner.Average && _active.Count > 0)
        {
        ...
        _tooltipContext = new BlazorChartTooltipContext
        {
            Title = combined.Title,
            Points = _active.OrderBy(a => a.DatasetIndex).Select(a => new BlazorChartTooltipPoint
            {
                DatasetIndex = a.DatasetIndex,
                DataIndex = a.DataIndex,
                Label = a.SeriesLabel,
        ...
        // Highlight overlay.
        bool indexMode = tip.Mode is BlazorChartInteractionMode.Index or BlazorChartInteractionMode.X && !_scene.IsRadialOrCircular;
        if (indexMode && _scene.PlotArea is { } pa)
            _hoverNodes.Add(new BlazorChartSvgLine
            {
        ...
        {
            if (el.Shape is BlazorChartSvgCircle c)
                _hoverNodes.Add(new BlazorChartSvgCircle
                {
    ...
    private async Task OnClickElement(BlazorChartDataElement e)
    {
    ...
    private static BlazorChartSvgNode FocusOutline(BlazorChartDataElement el) => el.Shape switch
    {
        BlazorChartSvgRect r => new BlazorChartSvgRect { X = r.X - 2, Y = r.Y - 2, Width = r.Width + 4, Height = r.Height + 4, Fill = "none", Stroke = "#1a1a1a", StrokeWidth = 2, CssClass = "bc-focus-ring" },
        BlazorChartSvgCircle c => new BlazorChartSvgCircle { Cx = c.Cx, Cy = c.Cy, R = c.R + 5, Fill = "none", Stroke = "#1a1a1a", StrokeWidth = 2, CssClass = "bc-focus-ring" },
        _ => new BlazorChartSvgCircle { Cx = el.CenterX, Cy = el.CenterY, R = 8, Fill = "none", Stroke = "#1a1a1a", StrokeWidth = 2, CssClass = "bc-focus-ring" }
    };

    private string Describe(BlazorChartDataElement el)
    {
    ...
    private void ToggleLegend(BlazorChartLegendItemModel item)
    {
    ...
    private string ViewBox => $"0 0 {BlazorChartSvg.N(_vw)} {BlazorChartSvg.N(_vh)}";
    ...
    private bool IsActive(BlazorChartDataElement e) => _active.Count > 0 && _active.Contains(e);
    ...
    private static string AlignToFlex(BlazorChartAlign a) => a switch
    {
        BlazorChartAlign.Start => "flex-start",
        BlazorChartAlign.End => "flex-end",
        _ => "center"
    };

    private static string TextAlign(BlazorChartAlign a) => a switch
    {
        BlazorChartAlign.Start => "left",
        BlazorChartAlign.End => "right",
        _ => "center"
    };
